    ...
    def render_waypoints(self, e):
        ...
        for i in range(points.shape[0]):
            if len(self.drawn_waypoints) < points.shape[0]:
                b = e.program.vertex_list(1, gl.GL_POINTS, batch=e.batch,
                    position=('f', [scaled_points[i, 0], scaled_points[i, 1], 0.]),
                    colors=('Bn', [183, 193, 222, 255]))
                self.drawn_waypoints.append(b)
            else:
                self.drawn_waypoints[i].position[:] = [scaled_points[i, 0], scaled_points[i, 1], 0.]
...
def main():
    ...
    def render_callback(env_renderer):
        ...
        x = e.cars[0].position[::3]
        y = e.cars[0].position[1::3]
    ...
    obs, info = env.reset(np.array([[conf.sx, conf.sy, conf.stheta]]))
    env.render()
